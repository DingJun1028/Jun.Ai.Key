...
class DriveManager {
    ...
    func signIn(presentingViewController: UIViewController, completion: @escaping (Result<Void, DriveManagerError>) -> Void) {
        ...
        GIDSignIn.sharedInstance.signIn(withPresenting: presentingViewController, hint: nil, additionalScopes: driveScopes) { signInResult, error in
            if let gidError = error { // This 'error' is of type Error (likely GIDSignInError)
                print("DriveManager: Google Sign-In error: \(gidError.localizedDescription)")
                completion(.failure(.apiError(gidError))) // Wrap GID error
                return
            }

            guard let user = signInResult?.user else {
                print("DriveManager: Google Sign-In error: User data not available.")
                completion(.failure(.userDataUnavailable)) // Already DriveManagerError
                return
            }
